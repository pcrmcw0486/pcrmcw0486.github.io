...
### ConfigData 로드 - application.yaml이 읽히는 과정

Spring이 자동으로 찾아서 읽는 파일 위치 (아래로 갈수록 우선순위 높음):
...
외부 config/application.yaml만 변경해서 JAR을 다시 빌드하지 않고 설정만 변경 등으로 운영한다던가..
...
- ApplicationContext가 생기기전에 동작하는 것으로 (Bean이 아님), spring.factories에 등록필요:
...
**spring.config.import vs EnvironmentPostProcessor:**

| | spring.config.import | EnvironmentPostProcessor |
|---|---|---|
| 방식 | 선언적 (yml 파일에 작성) | 코드로 직접 작성 |
| 소스 | 파일, URL, 외부 서비스에서 설정값 로드 | 어떤 소스든 가능 (커스텀 암호화 해제, 동적 계산 등) |
| 제약 | Spring이 지원하는 소스만 사용 가능 | 더 유연하지만 spring.factories 등록 필요 |

AWS Secrets가 spring.config.import로 지원되기 전에는 EnvironmentPostProcessor 방식으로 구현했음.
...
3. 커맨드라인: `java -jar app.jar --spring.profiles.active=prod`
...
Phase 05에서 Bean이 생성될 때 placeholder가 해소된다:
...
Phase01에서 등록한 인프라 리스너들(boot 리스너 Phase01참조)이 이벤트를 받아 처리한다.
ex) LoggingApplicationListener가 이 시점에 logging.level을 가져가서 로그 레벨을 세팅한다.
